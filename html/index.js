...
let jobMarkersDataTable = $("#job-markers-container").DataTable( {
	...
	"pageLength": 10,
	"createdRow": function ( row, data, index ) {
		$(row).find(".teleport-to-marker-btn").click(function(e) {
			e.stopPropagation();
			// Close tooltip of this button
			$(this).tooltip("hide");

			$.post(`https://${resName}/teleportToMarker`, JSON.stringify({ markerId: parseInt(data[0]) }));
		}).tooltip();
	...
	"columnDefs": [
        { 
			...
			<div class="d-inline-flex gap-3">
				<button class='btn py-0 px-2 teleport-to-marker-btn' style="background-color: #706fd3" data-bs-toggle="tooltip" data-bs-placement="top" title="TP"><i class=\"bi-geo-fill\"></i></button>
			</div>
			`
        },
...
$("#settings").submit(async function(event) {
	...
	let sharedSettings = {
		...
		resetEmployeeWorkTimeEveryNDays: parseInt( $("#settings_resetEmployeeWorkTimeEveryNDays").val() ),
	}
...
async function loadSettings(fullConfig) {
	...
	// Reset employee work time setting
	$("#settings_resetEmployeeWorkTimeEveryNDays").val(fullConfig.resetEmployeeWorkTimeEveryNDays);
}
